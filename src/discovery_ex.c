...
void led_and_button_example(void)
{
    ...
    while(1) {
        ...
        uint32_t test = GPIO_BUTTON->IDR;
        
        if ((test & BUTTON_PIN) == 1) {
        ...
        /* Man beachte hierbei die Klammerung des &-Terms. Diese Klammerung
           ...
           auf 1 setzen wollen. Wir verwenden also das BSRRL-Register: */
        	GPIO_LED->BSRR = (1 << LED_GREEN_PIN) | (1 << LED_ORANGE_PIN) | (1 << LED_RED_PIN) | (1 << LED_BLUE_PIN);
        ...
        /* Ist der Taster nicht gedrückt, wollen wir die IO-Pins 12 bis 15 des
           Ports D auf 0 setzen. Wir verwenden also das BSRRH-Register: */
        	GPIO_LED->BSRR = (1 << (LED_GREEN_PIN+16)) | (1 << (LED_ORANGE_PIN+16)) | (1 << (LED_RED_PIN+16)) | (1 << (LED_BLUE_PIN+16));
...
void led_and_timer_example(void)
{
    ...
    GPIOD->BSRR |= 0xA000; // Pin 13 und 15 ein
    GPIOD->BSRR |= 0x5000; // Pin 12 und 14 aus
...
void dma_pwm_led_example(void)
{
    ...
    DMA1->LISR &= 0xFF82FFFF;	// 1111 1111 1000 0010 1111 1111 1111 1111
    DMA1->HISR &= 0xF0FFF082;	// 1111 0000 1111 1111 1111 0000 1000 0010
